...
println("inside generate get_sk...")
...
def get_sk_values(response: String, post_url: String): List[PkSkGen] = {
  ...
  var b = ""
  var c = ""
  val pk_sk_values = jsonStrToMap(response).get("pks_sks").get
...
val out0 =
  if (
    Config.enable_sk_service != "false" && spark.conf.get("new_data_flag") == "true" && Config.sk_service_col != "None"
  ) {
    val sksConfig = SKDef(
    ...
    val temp_df = in0
      .where(
        sksConfig.skCol + " is null or " + sksConfig.skCol + " == '-1'"
      )
      .select(
        concat_ws("~|~",
          sksConfig.nkCol.map { x =>
            expr(x)
          }: _*
        )
      )
    ...
    val sk_outputs = temp_df.rdd
      .mapPartitions { rowPartition =>
        rowPartition.grouped(sk_service_batch_size).flatMap {
          rowGroup =>
            val sk_input = rowGroup.map(_.getString(0)).toList
            var sk_response_output =
              get_sk_values(get_sk_values_response(pipeline_name, sk_input, post_url),
                post_url
              )
            var while_count = 0
            while (
    ...
    in0
      .withColumn(sksConfig.tableName + "_concat_pk",
        concat_ws("~|~",
          sksConfig.nkCol.map { nk_col =>
            expr(nk_col)
          }: _*
        )
      )
      .as("in0")
      .join(sk_outputs.as("in1"),
        expr(
          "in0." + sksConfig.tableName + "_concat_pk == in1.concat_pk"
        ),
        "left"
      )
      .withColumn(sksConfig.skCol,
        when(col(sksConfig.skCol) === lit("-1") || col(
          sksConfig.skCol
        ).isNull,
          col("service_sk")
